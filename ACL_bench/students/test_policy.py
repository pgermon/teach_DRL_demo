...
from ACL_bench.teachers.teacher_controller import param_vec_to_param_dict

from ACL_bench.students.spinup.utils.test_policy import load_policy_and_env as spinup_load_policy
from ACL_bench.students.openai_baselines.ppo2.ppo2 import get_model as get_baselines_model
from ACL_bench.students.ppo_utils import create_custom_vec_normalized_envs
...
def load_fixed_test_set(save_path, test_set_name):
    teacher_dict = load_env_params(save_path)
    teacher_param_env_bounds = OrderedDict(teacher_dict["env_param_bounds"])
    test_param_vec = np.array(pickle.load(open("ACL_bench/teachers/test_sets/" + test_set_name + ".pkl", "rb")))
