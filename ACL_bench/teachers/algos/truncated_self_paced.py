...
class AbstractSelfPacedTeacher:
    ...
    def sample_trunc(self):
        ...
        cdf_u = multivariate_normal.cdf(self.bounds[1], self.mean, sigma)
        cdf_l = multivariate_normal.cdf(self.bounds[0], self.mean, sigma)
        # As long as there is a 5 percent change that a sample from a regular normal is in the bounds, 100 rejection
        # sampling iterations have only a change of 0.003% of failing
        if cdf_u - cdf_l >= 0.05:
    ...
    def sample_rejection(mu, sigma, bounds, n_samples=100, rejection_iters=20):
        ...
        if not rejection_success:
            print("Warning! Rejection sampling not successful! %d samples clipped!" % np.sum(violations))
